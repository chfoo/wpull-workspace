"""Localization aware date and time parsing.

This module provides date and time parsing for formats that may be
produced during directory listings.
"""
import datetime
import json
import os.path
...
# ↓ Automatically generated. See the CLDR section below.
...
def guess_datetime_format(lines: Iterable[str], threshold: int=5) \
    ...
    """Guess the order of the year, month, day and 12/24 hour of some lines.

    The lines are typically raw lines from a directory listing.
...
def parse_datetime(text: str, date_format: str=None,
    ...
    The line is typically a line from a directory listing.

    Returns:
        A tuple containing

        1. datetime object
        2. start index of where the date is parsed
        3. end index of where the date is parsed
    """
...
def y2k(year: int) -> int:
    ...
    This uses a simple rule where 69 or greater is interpreted as
    1900s. Any other year is considered 2000s.
    """
...
def _parse_cldr_json(directory, language_codes=_DEFAULT_LANGUAGE_CODES,
                     massage=True):
    """Parse CLDR JSON datasets to for date time things.

    This function is how the tables are generated in the source code.
    """
