"""Multi-platform file directory listing parser.

This module provides parsing human-readable directory listings
for MSDOS/Windows and Linux/Unix with support of other locales
other than English. It also supports parsing the NLST format as well.
"""
import enum
import re
import itertools
from typing import NamedTuple, Optional, Iterable, TextIO

import datetime

from .date import parse_datetime
import wbull.protocol.ftp.ls.date


class FileType(enum.Enum):
    """Type of file in directory listing."""
    file = 'file'
    directory = 'dir'
    symlink = 'symlink'
    other = 'other'
...
Attributes:
    ...
    type: Type of file.
    size: Size of file.
    date: A datetime object in UTC.
    dest: Destination filename for symlinks.
    perm: Unix permissions expressed as an integer.
"""
...
class LineParser(object):
    """Parse individual lines in a directory listing."""
    def __init__(self):
        self.type = None
    ...
    def parse_nlst(self, lines):
        ...
        for line in lines:
            yield FileEntry(line, None, None,
                            None, None, None)
    ...
    def parse_msdos(self, lines):
        ...
        for line in lines:
            ...
            yield FileEntry(filename, file_type, file_size,
                            file_datetime, None, None)

    def parse_unix(self, lines):
        """Parse lines from a Unix ls command format."""
...
def guess_listing_type(lines: Iterable[str], threshold: int=100) -> str:
    """Guess the style of directory listing.

    Args:
        lines: A directory listing with each item in a line.
        threshold: Maximum number of lines to sample.

    Returns:
        ``unix``, ``msdos``, ``nlst``, ``unknown``.
    """
    scores = {
        'unix': 0,
        'msdos': 0,
...
def parse_unix_perm(text: str) -> int:
    """Parse a Unix permission string and return integer value.

    Example: ``r--------`` will return ``256``.
    """
...
class ListingParser(LineParser):
    ...
    """
    def __init__(self, text: Optional[str]=None, file: Optional[TextIO]=None):
        super().__init__()

        self._text = text
        self._file = file

    def parse_input(self) -> Iterable[FileEntry]:
        """Parse the listings and return the file entries.
        """
        ...
        datetime_format = wbull.protocol.ftp.ls.date.guess_datetime_format(
            sample_lines)
